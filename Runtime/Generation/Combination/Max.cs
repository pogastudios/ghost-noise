...
namespace Packages.GhostNoise.Generation.Combination
{
	...
	public class Max:Generator
	{
		...
		public override float GetValue(float x, float y, float z) => Mathf.Max(_genA.GetValue(x, y, z), _genB.GetValue(x, y, z));
		public override float GetValue(Vector3 p) => Mathf.Max(_genA.GetValue(p), _genB.GetValue(p));
	}
}